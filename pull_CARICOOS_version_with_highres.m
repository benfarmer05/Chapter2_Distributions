%% Enhanced Script to pull CARICOOS SWAN wave data from multiple sources
...
% CUSTOM_START_DATE = '2024-01-03'; CUSTOM_END_DATE = '2024-01-05';    % 3 days test (~2-3 min)
% CUSTOM_START_DATE = '2024-01-03'; CUSTOM_END_DATE = '2024-01-09';    % 7 days test (~5-7 min)  
% CUSTOM_START_DATE = '2024-01-03'; CUSTOM_END_DATE = '2024-02-02';    % 31 days test (~15-20 min)
% CUSTOM_START_DATE = '2024-01-03'; CUSTOM_END_DATE = '2025-08-17';    % Full run (~4-6 hours)
...
if USE_CUSTOM_DATES
    ...
    estimated_files = estimated_days * 4; % 4 datasets
    if estimated_days <= 3
        fprintf('Quick test: %d days, %d files, ~%.0f minutes\n', estimated_days, estimated_files, estimated_files * 0.5);
    elseif estimated_days <= 10
        fprintf('Weekly test: %d days, %d files, ~%.0f minutes\n', estimated_days, estimated_files, estimated_files * 0.75);
    elseif estimated_days <= 40
        fprintf('Monthly test: %d days, %d files, ~%.0f minutes\n', estimated_days, estimated_files, estimated_files * 1.0);
    else
        fprintf('Full production: %d days, %d files, ~%.0f hours\n', estimated_days, estimated_files, estimated_files * 1.0 / 60);
    end
else
...
fprintf('  Total files: %d (4 datasets × %d days)\n', total_days_to_download * 4, total_days_to_download);
...
for dataset_idx = 1:total_datasets
    ...
    dataset_data.times = [];
    dataset_data.first_day = true;
    
    % Calculate days to download based on overlap period
    ...
    % Create array of all days to download
    all_days = overlap_start_date:days(1):overlap_end_date;
    ...
    % Download data DAY BY DAY within the overlap period
    try
        for day_idx = 1:total_days
            current_day_count = current_day_count + 1;
            day_start_time = tic;
            
            current_date = all_days(day_idx);
            
            % Update progress bar
            progress = current_day_count / total_days;
            status_msg = sprintf('%s: %s (%d/%d days)', ...
                current_dataset.name, datestr(current_date, 'yyyy-mm-dd'), current_day_count, total_days);
            waitbar(progress, h, status_msg);
            
            % Construct daily URL
            start_date = sprintf('%sT00:00:00Z', datestr(current_date, 'yyyy-mm-dd'));
            end_date = sprintf('%sT23:59:59Z', datestr(current_date, 'yyyy-mm-dd'));
            ...
            if length(lat_range) == 2 && length(lon_range) == 2
                ...
                % Daily time steps (3-hourly data = 8 per day)
                approx_time_steps = 8;
                fprintf('  Estimated time steps: %d (3-hourly)\n', approx_time_steps);
                ...
                if approx_size_mb > 50
                    fprintf('  NOTE: Larger daily file - may take longer\n');
                elseif approx_size_mb > 100
                    fprintf('  WARNING: Large daily file - may cause timeout!\n');
                end
            end
            fprintf('  ==============================\n\n');
            
            % Download with retry logic
            filename = sprintf('%s_%s.nc', current_dataset.name, datestr(current_date, 'yyyymmdd'));
            download_success = false;
            retry_count = 0;
            max_retries = 10; % Reduced retries for large files
                ...
                try
                    ...
                    if retry_count < max_retries
                        ...
                        if contains(ME.message, 'timeout') || contains(ME.message, 'time')
                            fprintf('    → Likely cause: File too large for server timeout\n');
                            fprintf('    → Suggestion: Try smaller time chunks or spatial subset\n');
            ...
            try
                for var_idx = 1:length(current_dataset.variables)
                    var_name = current_dataset.variables{var_idx};
                    daily_data = ncread(filename, var_name);
                    
                    if dataset_data.first_day
                        dataset_data.(var_name) = daily_data;
                    else
                        % Concatenate along time dimension (3rd dimension)
                        dataset_data.(var_name) = cat(3, dataset_data.(var_name), daily_data);
                    end
                end
                
                % Read time data
                daily_time = ncread(filename, 'time');
                if dataset_data.first_day
                    dataset_data.times = daily_time(:);
                    ...
                    dataset_data.first_day = false;
                else
                    dataset_data.times = [dataset_data.times; daily_time(:)];
                end
                
                fprintf('  Day %s: Added %d time steps (Total so far: %d)\n', ...
                    datestr(current_date, 'yyyy-mm-dd'), length(daily_time), length(dataset_data.times));
            ...
            % Show progress
            day_time = toc(day_start_time);
            fprintf('  Progress: %d/%d days (%.1f%%) | File time: %.1f sec\n', ...
                current_day_count, total_days, progress*100, day_time);
        end
