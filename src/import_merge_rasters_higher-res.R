  ...
  #pull NOAA crm, produced in ~2019 (maybe ? from Dr. Daniel Holstein) from presumably NOAA bathymetry database (similar to https://www.ncei.noaa.gov/maps/grid-extract/)
  # CRS: 26920 (NAD83 / UTM zone 20N)
  # 30 m max resolution
  # 
  # NOTE - Could also be used for USVI region in a backup merge, since it has nice resolution generally, less artifacts than the crm below, and full extent across Anegada
  ...
  #   -	West: -68.022
  #
  # NOTE - This could be used for PR in backup merge, because it has seamless projection with the above when crossing from STT to Culebra/Vieques.
  # NOTE - Would simply use this for entire PR/USVI domain extent, but above crm happens to be a bit better for our use case in the USVI
  ...
  # # DEPRECATED VERSION WHERE MODELING IS LIMITED TO VIEQUES EASTWARD
  ...
  # ACTIVE VERSION WHERE MODELING COVERS ENTIRE US CARIBBEAN
  #
  # First crop to reduce processing time (gets the right dimensions)
  ...
  # NOTE - return here if it ever appears that more NAs were created around land than necessary
  ...
  # Resample the landmask to match Blondeau resolution
  ...
  # Add missing Mona Island landmask and resample
  ...
  # NOTE - this patch of higher-resolution bathymetry came from a ~2019 CRM NOAA export supplied by
  #         Dr. Daniel Holstein. Also the same raster from which BVI data are tacked on below
  ...
  # NOTE - for some reason, BVI bathymetry data was available/best in a NOAA CRM export version that was made years ago
  #         (potentially 2019, pulled by Dr. Daniel Holstein). Used that here to add BVI bathy to dataset
  ...
  values(bathy_crm_cleaned)[values(bathy_crm_cleaned) >= 0] = NA  #remove land
  values(bathy_crm_cleaned)[values(bathy_crm_cleaned) <= deepest_crm] = NA  
  
  # Remove CRM data where Blondeau already has bathymetry (we don't want to overwrite it)
  ...
  # Very high-resolution 2014 LIDAR data for a small patch of area surrounding Anegada
  ...
  # Needed to address issues where negative sea level water bodies (mainly ponds) showed up in the
  #   bathy, to avoid any potential resulting issues
  
  # NOTE - since the below is done POST-resampling to 50 m, some inland water bodies that
  #         truly are connected to the ocean through small channels likely are lost (and set to
  #         "land"). would be a huge computational task to try and do this at native resolution
  ...
  water_binary[water_binary > 0] = NA
  water_binary[water_binary <= 0] = 1
  ...
  # Secondary memory-efficient depth-based function
  ...
  # Make use of existing defined water patches and further refine
  min_patch_size = 10000  #or whatever threshold is needed; settled on this
  ...
  ocean_mask[!water_patches %in% large_patches] = NA
  ocean_mask[water_patches %in% large_patches] = 1
  ...
  # ################################## Merge 1: PR to MCD ##################################
  # 
  # # NOTE - the below 3 merges were for a "back-up" version of merged bathymetry that didn't have the artifact
