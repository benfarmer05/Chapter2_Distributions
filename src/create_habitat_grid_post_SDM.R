...
grid_low_values <- extract(susc_rasters$low, grid_SDMreef_no_land, fun = mean, na.rm = TRUE)
grid_moderate_values <- extract(susc_rasters$moderate, grid_SDMreef_no_land, fun = mean, na.rm = TRUE)
grid_high_values <- extract(susc_rasters$high, grid_SDMreef_no_land, fun = mean, na.rm = TRUE)

# Add to grid
grid_SDMreef_no_land$low_coral_cover <- grid_low_values[, 2]
grid_SDMreef_no_land$moderate_coral_cover <- grid_moderate_values[, 2]
grid_SDMreef_no_land$high_coral_cover <- grid_high_values[, 2]

# Validate that susceptibility covers sum to total cover
cat("\n=== GRID SUSCEPTIBILITY COVER VALIDATION ===\n")
susc_sum <- grid_SDMreef_no_land$low_coral_cover + 
  grid_SDMreef_no_land$moderate_coral_cover + 
  grid_SDMreef_no_land$high_coral_cover
diff <- grid_SDMreef_no_land$mean_coral_cover - susc_sum
