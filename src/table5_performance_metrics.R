  ...
  for(species in names(all_results)) {
    ...
    # Average across folds, then calculate r² from averaged r
    cv_avg_raw <- colMeans(cv_metrics_raw, na.rm = TRUE)
    cv_avg_raw["r2"] <- cv_avg_raw["r"]^2  # Recalculate r² as square of averaged r
    cv_results_raw_list[[species]] <- cv_avg_raw
    
    cv_avg_calib <- colMeans(cv_metrics_calib, na.rm = TRUE)
    cv_avg_calib["r2"] <- cv_avg_calib["r"]^2  # Recalculate r² as square of averaged r
    cv_results_calib_list[[species]] <- cv_avg_calib
  ...
  cat("      r = Pearson correlation; r² = r-squared (square of averaged r); R² = coefficient of determination\n")
