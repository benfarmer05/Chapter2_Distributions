  ...
  # 
  # # Add to dataframe
  # combined_benthic_data_averaged$bathymetry <- species_env_values$bathymetry
  # combined_benthic_data_averaged$slope <- species_env_values$slope
  # 
  # # Add Y/N columns for bathymetry and slope presence
  # combined_benthic_data_averaged$bathymetry_present <- ifelse(is.na(combined_benthic_data_averaged$bathymetry), "N", "Y")
  # combined_benthic_data_averaged$slope_present <- ifelse(is.na(combined_benthic_data_averaged$slope), "N", "Y")
  # 
  # # Check how many NAs you have
  # # NOTE - this is actually kind of a lot.
  # sum(is.na(combined_benthic_data_averaged$bathymetry))
  ...
  # }
  # 
  # model_data <- orbicellids[complete.cases(orbicellids[, c("bathymetry", "slope", "cover")]), ]
  # 
  # # Check the new cover distribution
  # summary(model_data$cover)
  # prop_zeros_new <- sum(model_data$cover == 0) / nrow(model_data)
  ...
  # Add all variables to your filtered dataset
  orbicellids$bathymetry <- species_env_complex$bathymetry
  ...
  orbicellids$roughness <- species_env_complex$roughness
  orbicellids$VRM <- species_env_complex$VRM
  orbicellids$max_curv <- species_env_complex$max_curv
  orbicellids$mean_curv <- species_env_complex$mean_curv
  orbicellids$planform_curv <- species_env_complex$planform_curv
  orbicellids$profile_curv <- species_env_complex$profile_curv
  ...
  # ################################## try just orbicellids ##################################
  # 
  # # Streamlined GAM Analysis for Coral Cover - Linear Script
  # 
  # library(mgcv)
  # library(dplyr)
  # library(ggplot2)
  ...
  # # 1. DATA PREPARATION
  # 
  # # Filter for Orbicella species
  # orbicellids <- combined_benthic_data_averaged %>%
  #   filter(grepl("Orbicella", spp))
  # 
  # cat("Total Orbicella observations:", nrow(orbicellids), "\n")
  # 
  # # Transform coordinates to UTM
  # species_coords <- vect(as.data.frame(orbicellids), 
  #                        geom = c("lon", "lat"), 
  #                        crs = "EPSG:4326")
  # species_coords_utm <- project(species_coords, crs(env_complex))
  # 
  # # Extract all environmental variables
  # env_values <- terra::extract(env_complex, species_coords_utm)
  # 
  # # Combine with species data
  # model_data <- cbind(orbicellids, env_values[, -1])  # Remove ID column
  # 
  # # Remove rows with missing environmental values
  # model_data <- model_data[complete.cases(model_data), ]
  # 
  # cat("Observations with complete environmental data:", nrow(model_data), "\n")
  # cat("Cover summary:\n")
  # print(summary(model_data$cover))
  # 
  # # 2. COLLINEARITY CHECK & VARIABLE SELECTION
  # 
  # # Define environmental variables
  # env_vars <- c("bathymetry", "aspect", "slope", "TPI", "roughness", "VRM",
  #               "max_curv", "mean_curv", "planform_curv", "profile_curv")
  # 
  # # Calculate correlation matrix
  # cor_matrix <- cor(model_data[env_vars], use = "complete.obs")
  # 
  ...
  # }
  # 
  # # Select representative variables from each category to avoid collinearity
  # # Choose one from each conceptual group
  # selected_vars <- c(
  #   "bathymetry",      # Depth
  #   "TPI",             # Topographic position  
  #   "VRM",             # Surface complexity
  #   "planform_curv"    # Curvature
  # )
  # 
  # cat("\nSelected variables for modeling:", paste(selected_vars, collapse = ", "), "\n")
  # 
  # # Check correlations among selected variables
  # cat("\nCorrelations among selected variables:\n")
  # print(round(cor_matrix[selected_vars, selected_vars], 3))
  # 
  # # 3. SYSTEMATIC GAM FITTING
  # 
  # # Define model formulas of increasing complexity
  # model_formulas <- list(
  #   null = cover ~ 1,
  #   depth_only = cover ~ s(bathymetry),
  #   additive = cover ~ s(bathymetry) + s(TPI) + s(VRM) + s(planform_curv),
  #   spatial = cover ~ s(bathymetry) + s(TPI) + s(VRM) + s(planform_curv) + s(lon, lat),
  #   interaction = cover ~ s(bathymetry) + s(TPI) + s(VRM) + s(planform_curv) + s(bathymetry, TPI)
  # )
  # 
  ...
  #   
  #   model_results[[dist_name]] <- list()
  #   
  #   for(model_name in names(model_formulas)) {
  #     cat("  Model:", model_name, "\n")
  #     
  #     # Prepare data based on distribution
  #     if(dist_name == "gamma") {
  #       fit_data <- model_data[model_data$cover > 0, ]  # Remove zeros for gamma
  #     } else {
  #       fit_data <- model_data
  #     }
  #     
  #     # Fit model
  #     tryCatch({
  #       model <- gam(model_formulas[[model_name]], 
  #                    data = fit_data, 
  #                    family = distributions[[dist_name]])
  #       
  #       model_results[[dist_name]][[model_name]] <- model
  #       
  ...
  #         n_parameters = length(model$coefficients),
  #         n_observations = nrow(fit_data)
  #       ))
  #       
  #     }, error = function(e) {
  #       cat("    Error fitting model:", e$message, "\n")
  #       model_results[[dist_name]][[model_name]] <- NULL
  #     })
  #   }
  # }
  # 
  ...
  # 
  # # Get best model
  # best_dist <- model_comparison$distribution[1]
  # best_model_name <- model_comparison$model[1]
  # best_model <- model_results[[best_dist]][[best_model_name]]
  # 
  # cat("\nBest model:", best_model_name, "with", best_dist, "distribution\n")
  # cat("AIC:", round(model_comparison$AIC[1], 2), "\n")
  # cat("Deviance explained:", round(model_comparison$deviance_explained[1], 1), "%\n")
  # 
  ...
  # 
  # # 5. VISUALIZATION OF BEST MODEL
  # 
  # cat("\nCreating prediction plots...\n")
  # 
  # # Set up prediction data for each variable
  # plots <- list()
  # 
  # for(var in selected_vars) {
  #   
  #   # Create prediction sequence
  #   var_range <- seq(min(model_data[[var]], na.rm = TRUE), 
  ...
  #     bathymetry = median(model_data$bathymetry, na.rm = TRUE),
  #     TPI = median(model_data$TPI, na.rm = TRUE),
  #     VRM = median(model_data$VRM, na.rm = TRUE),
  #     planform_curv = median(model_data$planform_curv, na.rm = TRUE)
  #   )
  #   
  ...
  #   pred_data[[var]] <- var_range
  #   
  #   # Add spatial coordinates if needed (use medians)
  #   if("spatial" %in% best_model_name || "interaction" %in% best_model_name) {
  #     pred_data$lon <- median(model_data$lon, na.rm = TRUE)
  #     pred_data$lat <- median(model_data$lat, na.rm = TRUE)
  #   }
  #   
  #   # Get predictions
  #   pred <- predict(best_model, pred_data, se.fit = TRUE, type = "response")
  #   
  #   # Create plot data
  #   plot_df <- data.frame(
  #     x = var_range,
  ...
  #     geom_line(aes(y = fitted), color = "steelblue", size = 1.2) +
  #     geom_point(data = model_data, aes_string(x = var, y = "cover"), 
  #                alpha = 0.3, size = 0.8, color = "gray40") +
  #     labs(x = var, y = "Predicted coral cover (%)", 
  #          title = paste("Effect of", var, "on coral cover")) +
  #     theme_minimal() +
  ...
  #   model_data$residuals <- residuals(best_model)
  #   
  #   # Simple spatial plot of residuals
  #   plot(model_data$lon, model_data$lat, col = ifelse(model_data$residuals > 0, "red", "blue"),
  #        pch = 16, cex = abs(model_data$residuals)/max(abs(model_data$residuals), na.rm = TRUE),
  #        main = "Spatial distribution of model residuals",
  #        xlab = "Longitude", ylab = "Latitude")
  #   legend("topright", c("Positive", "Negative"), col = c("red", "blue"), pch = 16)
  # }
  # 
  # # Variable importance (approximate)
  # cat("\nVariable importance (based on chi-square):\n")
  ...
  # # Final summary
  # cat("\n=== ANALYSIS COMPLETE ===\n")
  # cat("Best performing model:", best_model_name, "with", best_dist, "family\n")
  # cat("Final dataset size:", nrow(model_data), "observations\n")
  # cat("Variables included:", paste(selected_vars, collapse = ", "), "\n")
  ################################## make predictions ##################################
